...
local function updateQuestsTable()
    ...
    if( not JSON_String )then print("SV_QUESTS.LUA ERROR: COULDN'T OPEN/READ QUESTS JSON FILE") return end

    local QuestsTable = util.JSONToTable(JSON_String)
    PrintTable(QuestsTable.Tasks)
end

updateQuestsTable()

local function sendQuestsTableUpdate( ply, table )
    net.Start("PRSBOX_QUESTS_Update", false)

    local TempTable = getQuestsTableFor( ply )

    net.WriteString(TempTable[ply:Name(), "Task1_desc"])
    net.WriteBool(TempTable[ply:Name(), "Task1_isDone"])

    net.WriteString(TempTable[ply:Name(), "Task2_desc"])
    net.WriteBool(TempTable[ply:Name(), "Task2_isDone"])

    net.WriteString(TempTable[ply:Name(), "Task3_desc"])
    net.WriteBool(TempTable[ply:Name(), "Task3_isDone"])

    net.Send( ply )
end

-- Returns table of quests for specified player
local function getQuestsTableFor( ply )
    local TempTable = {}

    if( doesPlayerHasQuests( ply ) )then
        TempTable = generateQuestsFor( ply )
    else
        TempTable = PlayersQuestsTable[ply:Name()]
    end
...
-- Generates quests table for given player
local function generateQuestsFor( ply )
    local Task1ID, Task2ID Task3ID = nil, nil, nil

    local BackupIterator = 0 -- To avoid crashing/inf loops with syntax errors in json.
    local CurrentTask = 1

    while(Task1ID!=nil and Task2ID!=nil and Task3ID!=nil and BackupIterator<1000) do
        BackupIterator++

        local TaskID = math.random(1, table.Count(QuestsTable.Tasks))

        if( TaskID==Task1ID or TaskID==Task2ID or TaskID==Task3ID )then continue end

        local TaskStr = "Task"+tostring(CurrentTask)
        PlayersQuestsTable[ply:Name(), TaskStr+"_desc"] = 
        PlayersQuestsTable[ply:Name(), TaskStr+"_isDone"] = false
        PlayersQuestsTable[ply:Name(), TaskStr+"_CurrentProgress"] = 
    end

    if( BackupIterator==1000 )then print("generateQuestsFor ERROR: Couldn't generate tasks for "+ply:Name()) return end
end

function doesPlayerHasQuests( ply )
    if( PlayersQuestsTable[ply:Name()]==nil )then return false 
